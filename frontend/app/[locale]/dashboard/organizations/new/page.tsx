...
export default function NewOrganizationPage() {
  ...
  const createMutation = useMutation((data: typeof formData) =>
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-2 text-sm text-muted-foreground">
        ...
        <span>/</span>
        <span>{locale === "ar" ? "جهة جديدة" : "New Organization"}</span>
      </div>

      <div>
        <h1 className="text-2xl font-bold tracking-tight">
          {locale === "ar" ? "إضافة جهة جديدة" : "Add New Organization"}
        </h1>
        <p className="text-muted-foreground">
          {locale === "ar"
            ? "سجل جهة حكومية جديدة في النظام"
            : "Register a new government entity in the system"}
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="flex justify-end gap-3 mt-6">
          ...
          <Button
            type="submit"
            disabled={createMutation.loading || !formData.name_en || !formData.name_ar}
          >
